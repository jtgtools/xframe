...
function addPlanarFrame(
  builder: ModelBuilder,
  id: string,
  start: string,
  end: string,
  theory: FrameTheoryInput = { kind: "euler-bernoulli" },
): ModelBuilder {
...
function cantileverCase(options: {
  ...
  readonly load: {
    ...
    readonly moment?: readonly [number, number, number];
  };
  ...
  return validationCase(
    ...
    () => {
      ...
      const result = solve(
        builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", ...options.load }] }),
      );
      return [
        value(
          options.dof,
          options.reference,
          displacement(result, "b", options.dof),
          options.units,
          options.reference,
        ),
      ];
    ...
    () => {
      ...
      return [
        ...
        value(
          ...
          Math.abs(reaction(result, "a", "rz")),
          "N*m",
        ),
        value(
          "right fixed-end moment magnitude",
          Math.abs(load * length) / 8,
          Math.abs(reaction(result, "c", "rz")),
          "N*m",
        ),
      ];
    },
  ...
  validationCase(
    ...
    () => {
      ...
      return [
        ...
        value(
          "support moment magnitude",
          Math.abs(load * overhang),
          Math.abs(frameEndForce(result, "f1", 11)),
          "N*m",
        ),
      ];
    },
  ...
  validationCase(
    {
      ...
      supports:
        "Three simple supports; first support restrains axial translation; planar unused DOFs restrained.",
  ...
  validationCase(
    {
      ...
      supports:
        "Four simple supports; first support restrains axial translation; planar unused DOFs restrained.",
    ...
    () => {
      ...
      pin(builder, "c");
      pin(builder, "d");
      const loads = ["f1", "f2", "f3"].map((frameId) => ({
        kind: "member-distributed" as const,
        frameId,
        coordinateSystem: "local" as const,
        startIntensity: [0, w, 0] as const,
      ...
      return [
        ...
        value(
          "support moment c magnitude",
          (Math.abs(w) * length ** 2) / 10,
          Math.abs(frameEndForce(result, "f2", 11)),
          "N*m",
        ),
      ];
    },
  ...
  validationCase(
    ...
    () => {
      ...
      const builder = prismaticFrameBuilder({
        ...
        shearAreaY: shearArea,
        shearAreaZ: shearArea,
        ...
        outputs.push(
          displacement(
            solve(
              builder.addLoadCase({
                id: "LC",
                loads: [{ kind: "nodal", nodeId: "b", moment: [0, 0, moment] }],
              }),
            ),
            "b",
            "rz",
  ...
  validationCase(
    ...
    () => {
      ...
      for (const ratio of ratios) {
        ...
        if (!(excess < previousExcess))
          throw new Error(`Timoshenko excess did not decrease at L/d=${ratio}.`);
  ...
  validationCase(
    {
      ...
      category: 2,
      description:
        "Explicit effective shear areas for rectangular, circular, and wide-flange sections are consumed once",
      ...
      frame3ddCoverage: "direct",
      notes:
        "xframe accepts effective shear area; it does not infer section shape. Published factors are supplied by the caller.",
    },
    () => {
      const length = 1.2;
      const area = 0.08;
      ...
      return [
        ...
        const builder = prismaticFrameBuilder({
          ...
          inertiaZ: inertia,
          torsionalConstant: J,
          shearAreaY: shearArea as number,
          shearAreaZ: shearArea as number,
          theory: { kind: "timoshenko" },
        });
        constrain(builder, "a", allDofs);
        const actual = displacement(
...
function unitAction(dof: DofName): {
  readonly force?: readonly [number, number, number];
  readonly moment?: readonly [number, number, number];
} {
          ...
          () => {
            ...
            const actual = displacement(
              solve(
                builder.addLoadCase({
                  id: "LC",
                  loads: [{ kind: "nodal", nodeId: loadedNode, ...unitAction(dof) }],
                }),
              ),
              loadedNode,
              dof,
            ...
            return [
              value(`${loadedNode}.${dof}`, reference, actual, dof.startsWith("r") ? "rad" : "m"),
            ];
          },
        ),
      );
