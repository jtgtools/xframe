...
function frameBuilder(length: number, releases?: { readonly start?: readonly DofName[]; readonly end?: readonly DofName[] }): ModelBuilder {
...
const springCases: ValidationCase[] = [
  validationCase({
    ...
    const builder = createModelBuilder()
      ...
      .addFrameSection({ id: "s", area: A, torsionalConstant: J, momentOfInertiaY: IY, momentOfInertiaZ: IZ })
    ...
    fixed(builder);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "c", force: [0, load, 0] }] }));
    const beamDeflection = (load * length ** 3) / (3 * E * IZ);
    return [
      value("load-node ty", beamDeflection + load / stiffness, displacement(output, "c", "ty"), "m"),
  ...
  validationCase({
    ...
    const builder = frameBuilder(length).addSpring({ id: "kr", startNodeId: "a", stiffness: { rz: rotationalStiffness } });
  ...
  validationCase({
    ...
    return [
      ...
      value("spring k1 force", Math.abs(k1 * deflection), Math.abs(springForce(output, "k1", 1)), "N"),
      value("spring k2 force", Math.abs(k2 * deflection), Math.abs(springForce(output, "k2", 1)), "N"),
      value("reaction ratio k2/k1", k2 / k1, Math.abs(springForce(output, "k2", 1) / springForce(output, "k1", 1)), "1"),
    ];
  }),
  validationCase({
    id: "VAL-07-004",
    ...
    function solveFor(stiffness: number): number {
      const builder = frameBuilder(length).addSpring({ id: "kr", startNodeId: "b", stiffness: { rz: stiffness } });
      fixed(builder);
      return displacement(solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force: [0, load, 0] }] })), "b", "ty");
    }
  ...
  validationCase({
    ...
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "n", force: [9_000, 3_000, 0] }] }));
    return [
...
const releaseCases: ValidationCase[] = [
  validationCase({
    ...
    fixed(builder, "a"); fixed(builder, "b");
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem: "local", startIntensity: [0, w, 0], endIntensity: [0, w, 0] }] }));
    return [
      value("released Mz", 0, frameEndForce(output, "f", 11), "N*m", Math.abs(w) * length ** 2),
      value("fixed-end Mz magnitude", Math.abs(w) * length ** 2 / 8, Math.abs(frameEndForce(output, "f", 5)), "N*m"),
  ...
  validationCase({
    ...
    fixed(builder, "a"); fixed(builder, "b");
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem: "local", startIntensity: [0, 0, w], endIntensity: [0, 0, w] }] }));
    return [
      value("released My", 0, frameEndForce(output, "f", 10), "N*m", Math.abs(w) * length ** 2),
      value("fixed-end My magnitude", Math.abs(w) * length ** 2 / 8, Math.abs(frameEndForce(output, "f", 4)), "N*m"),
      value("released-end reaction magnitude", 3 * Math.abs(w) * length / 8, Math.abs(reaction(output, "b", "tz")), "N"),
    ];
  }),
  validationCase({
    id: "VAL-08-003",
    ...
    fixed(builder, "a"); fixed(builder, "b");
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem: "local", startIntensity: [0, w, w], endIntensity: [0, w, w] }] }));
    return [
      value("start released Mz", 0, frameEndForce(output, "f", 5), "N*m", Math.abs(w) * length ** 2),
      value("end released Mz", 0, frameEndForce(output, "f", 11), "N*m", Math.abs(w) * length ** 2),
      value("start fixed My magnitude", Math.abs(w) * length ** 2 / 12, Math.abs(frameEndForce(output, "f", 4)), "N*m"),
      value("end fixed My magnitude", Math.abs(w) * length ** 2 / 12, Math.abs(frameEndForce(output, "f", 10)), "N*m"),
    ];
  }),
  validationCase({
    id: "VAL-08-004",
    ...
    fixed(builder, "a"); fixed(builder, "b");
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-point-moment", frameId: "f", coordinateSystem: "local", positionRatio: 0.5, moment: [torque, 0, 0] }] }));
    return [
      value("released end torsion", 0, frameEndForce(output, "f", 9), "N*m", torque),
      value("retained end torsion magnitude", torque, Math.abs(frameEndForce(output, "f", 3)), "N*m"),
    ];
  }),
  validationCase({
    id: "VAL-08-005",
    ...
    supports: "Start translations fixed; end transverse translations fixed; rotations hard-restrained only to remove inactive rigid modes.",
    ...
    return [
      value("axial displacement", load * length / (E * A), displacement(output, "b", "tx"), "m"),
      value("axial force magnitude", load, Math.abs(frameEndForce(output, "f", 0)), "N"),
      value("maximum released action", 0, Math.max(...[3, 4, 5, 9, 10, 11].map((index) => Math.abs(frameEndForce(output, "f", index)))), "N*m", load * length),
    ];
  }),
  validationCase({
    id: "VAL-08-006",
    ...
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem: "local", startIntensity: [0, w, 0], endIntensity: [0, w, 0] }] }));
    return [
      value("released end moment", 0, frameEndForce(output, "f", 11), "N*m", Math.abs(w) * length ** 2),
  ...
  validationCase({
    ...
    fixed(builder);
    builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force: [0, -1_000, 0] }] });
    let detected = 0;
    try {
...
const loadCases: ValidationCase[] = [
  validationCase({
    ...
    const length = 3;
    const actions = [
      { id: "Fx", load: { kind: "nodal" as const, nodeId: "b", force: [10_000, 0, 0] as const }, dof: "tx" as const, expected: 10_000 * length / (E * A) },
      { id: "Fy", load: { kind: "nodal" as const, nodeId: "b", force: [0, -12_000, 0] as const }, dof: "ty" as const, expected: -12_000 * length ** 3 / (3 * E * IZ) },
      { id: "Fz", load: { kind: "nodal" as const, nodeId: "b", force: [0, 0, 14_000] as const }, dof: "tz" as const, expected: 14_000 * length ** 3 / (3 * E * IY) },
      { id: "Mx", load: { kind: "nodal" as const, nodeId: "b", moment: [6_000, 0, 0] as const }, dof: "rx" as const, expected: 6_000 * length / (G * J) },
      { id: "My", load: { kind: "nodal" as const, nodeId: "b", moment: [0, 7_000, 0] as const }, dof: "ry" as const, expected: 7_000 * length / (E * IY) },
      { id: "Mz", load: { kind: "nodal" as const, nodeId: "b", moment: [0, 0, -8_000] as const }, dof: "rz" as const, expected: -8_000 * length / (E * IZ) },
    ];
    let builder = frameBuilder(length);
    fixed(builder);
    for (const action of actions) builder = builder.addLoadCase({ id: action.id, loads: [action.load] });
    const analysis = prepareAnalysis(builder.finalize());
    return actions.map((action) => value(action.id, action.expected, displacement(analysis.solveCase(action.id), "b", action.dof), action.dof.startsWith("r") ? "rad" : "m"));
  }),
  ...[0.25, 1 / 3].map((ratio, index) => validationCase({
    ...
    const builder = frameBuilder(length);
    fixed(builder);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-point-force", frameId: "f", coordinateSystem: "local", positionRatio: ratio, force: [0, load, 0] }] }));
    return [
      value("tip deflection", load * position ** 2 * (3 * length - position) / (6 * E * IZ), displacement(output, "b", "ty"), "m"),
      value("tip rotation", load * position ** 2 / (2 * E * IZ), displacement(output, "b", "rz"), "rad"),
    ];
  })),
  validationCase({
    id: "VAL-09-004",
    ...
    fixed(builder);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-point-moment", frameId: "f", coordinateSystem: "local", distanceFromElasticStart: position, moment: [0, 0, moment] }] }));
    const frame = output.frames[0]!;
    const stations = frame.internalForces.filter(({ x }) => Math.abs(x - position) < 1e-12);
    return [
      value("moment jump magnitude", moment, Math.abs(stations[1]!.bendingZ - stations[0]!.bendingZ), "N*m"),
  ...
  validationCase({
    ...
    const global = tuple3([0, 1, 2].map((row) => axes.localToGlobal[row * 3]! * local[0] + axes.localToGlobal[row * 3 + 1]! * local[1] + axes.localToGlobal[row * 3 + 2]! * local[2]));
    function run(coordinateSystem: "local" | "global", intensity: readonly [number, number, number]) {
      const builder = prismaticFrameBuilder({ length, end, orientation: [0, 0, 1], elasticModulus: E, shearModulus: G, area: A, inertiaY: IY, inertiaZ: IZ, torsionalConstant: J });
      fixed(builder);
      return solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem, startIntensity: intensity, endIntensity: intensity }] }));
    }
    const localResult = run("local", local);
    const globalResult = run("global", global);
    return allDofs.map((dof) => value(`tip ${dof}`, displacement(localResult, "b", dof), displacement(globalResult, "b", dof), dof.startsWith("r") ? "rad" : "m", 1));
  }),
  validationCase({
    id: "VAL-09-006",
    category: 9,
    ...
    fixed(builder);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "member-distributed", frameId: "f", coordinateSystem: "local", startDistanceFromElasticStart: start, endDistanceFromElasticStart: end, startIntensity: [0, w0, 0], endIntensity: [0, w1, 0] }] }));
    return [
  ...
  validationCase({
    ...
    let builder = createModelBuilder()
      ...
      .addFrameSection({ id: "s", area: A, torsionalConstant: J, momentOfInertiaY: IY, momentOfInertiaZ: IZ })
      .addFrame({ id: "h", startNodeId: "a", endNodeId: "b", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 1, 0] })
      .addFrame({ id: "i", startNodeId: "c", endNodeId: "d", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 0, 1] });
    fixed(builder, "a"); fixed(builder, "c");
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "self-weight", gravity }] }));
    const values = [];
    for (const [nodeId, prefix] of [["a", "horizontal"], ["c", "inclined"]] as const) {
  ...
  validationCase({
    ...
    const udl = { kind: "member-distributed" as const, frameId: "f", coordinateSystem: "local" as const, startIntensity: [0, -2_000, 0] as const, endIntensity: [0, -2_000, 0] as const };
    builder = builder.addLoadCase({ id: "A", loads: [point] }).addLoadCase({ id: "B", loads: [udl] }).addLoadCase({ id: "AB", loads: [point, udl] });
    const analysis = prepareAnalysis(builder.finalize());
    const a = analysis.solveCase("A");
    const b = analysis.solveCase("B");
    const ab = analysis.solveCase("AB");
    const combined = combineResults("C", [{ result: a, factor: 1 }, { result: b, factor: 1 }]);
    return [
      value("tip ty", displacement(ab, "b", "ty"), combined.nodes.find(({ id }) => id === "b")!.displacements.find(({ dof }) => dof === "ty")!.value, "m", 1),
      value("base Mz", frameEndForce(ab, "f", 5), combined.frames.find(({ id }) => id === "f")!.localEndForces[5]!, "N*m", 1),
      value("full displacement max difference", 0, Math.max(...ab.fullDisplacements.map((entry, index) => Math.abs(entry - combined.fullDisplacements[index]!))), "m", 1),
    ];
  }),
];
