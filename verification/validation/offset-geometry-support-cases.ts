...
function nodeVector(result: ReturnType<typeof solve>, nodeId: string, dofs: readonly DofName[]): number[] {
...
const rigidOffsetCases: ValidationCase[] = [
  validationCase({
    ...
    return [
      value("tip ty", (load * clearLength ** 3) / (3 * E * IZ), displacement(output, "b", "ty"), "m"),
      value("tip rz", (load * clearLength ** 2) / (2 * E * IZ), displacement(output, "b", "rz"), "rad"),
    ];
  }),
  validationCase({
    id: "VAL-04-002",
    ...
    const offsetResult = solve(offsetBuilder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force: [0, load, 0] }] }));
    ...
    const clearResult = solve(clearBuilder.addLoadCase({
      id: "LC",
      loads: [{ kind: "nodal", nodeId: "b", force: [0, load, 0], moment: [0, 0, 0.75 * load] }],
    }));
    const frame = offsetResult.frames[0]!;
    return [
      value("elastic-end ty", displacement(clearResult, "b", "ty"), frame.localEndDisplacements[7]!, "m", displacement(clearResult, "b", "ty")),
      value("elastic-end rz", displacement(clearResult, "b", "rz"), frame.localEndDisplacements[11]!, "rad", displacement(clearResult, "b", "rz")),
      value("base moment", frameEndForce(clearResult, "f", 5), frameEndForce(offsetResult, "f", 5), "N*m", load * 3.75),
    ];
  }),
  validationCase({
    id: "VAL-04-003",
  ...
  validationCase({
    ...
    const actions = { kind: "nodal" as const, nodeId: "b", force: [2_000, -8_000, 3_000] as const, moment: [900, 400, -600] as const };
    const outputs = [
      prismaticFrameBuilder({ length: 4, elasticModulus: E, shearModulus: G, area: A, inertiaY: IY, inertiaZ: IZ, torsionalConstant: J }),
      cantileverWithOffsets(4, [0, 0, 0], [0, 0, 0]),
    ].map((builder) => {
      constrain(builder, "a", allDofs);
      return solve(builder.addLoadCase({ id: "LC", loads: [actions] }));
    });
    return allDofs.map((dof) => value(`b.${dof}`, displacement(outputs[0]!, "b", dof), displacement(outputs[1]!, "b", dof), dof.startsWith("r") ? "rad" : "m", 1));
  }),
  validationCase({
    id: "VAL-04-005",
    category: 4,
    ...
    return [
      value("released end moment", 0, frameEndForce(output, "f", 11), "N*m", Math.abs(w) * clearLength ** 2),
  ...
  validationCase({
    ...
    return [
      value("elastic start axial force magnitude", load, Math.abs(frameEndForce(output, "f", 0)), "N"),
      value("elastic start bending moment magnitude", load * eccentricity, Math.abs(frameEndForce(output, "f", 5)), "N*m"),
      value("elastic end bending moment magnitude", load * eccentricity, Math.abs(frameEndForce(output, "f", 11)), "N*m"),
    ];
  }),
];
...
const geometryCases: ValidationCase[] = [
  validationCase({
    ...
    const load = 30_000;
    const builder = prismaticFrameBuilder({ length, start, end, orientation: [0, 0, 1], elasticModulus: E, shearModulus: G, area: A, inertiaY: IY, inertiaZ: IZ, torsionalConstant: J });
    constrain(builder, "a", allDofs);
    const force = tuple3(axes.x.map((entry) => entry * load));
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force }] }));
    const global = nodeVector(output, "b", ["tx", "ty", "tz"]);
    const transverse = Math.hypot(...global.map((entry, index) => entry - axes.x[index]! * dot(global, axes.x)));
    return [
      value("axial displacement", (load * length) / (E * A), dot(global, axes.x), "m"),
      value("transverse leakage", 0, transverse, "m", (load * length) / (E * A)),
    ];
  }),
  ...[0, 45, 90, 180].map((angle, index) => validationCase({
    ...
    const builder = prismaticFrameBuilder({ length: 3.2, orientation, elasticModulus: E, shearModulus: G, area: A, inertiaY: IY, inertiaZ: IZ, torsionalConstant: J });
    constrain(builder, "a", allDofs);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force }] }));
    const global = nodeVector(output, "b", ["tx", "ty", "tz"]);
    return [value("local-y tip displacement", (load * 3.2 ** 3) / (3 * E * IZ), dot(global, axes.y), "m")];
  })),
  validationCase({
    id: "VAL-05-006",
    category: 5,
    ...
    supports: "Start translations fixed; end transverse motion constrained by two affine equations, axial motion free.",
    ...
    for (const [id, normal] of [["y", axes.y], ["z", axes.z]] as const) {
      builder = builder.addConstraint({
        id: `end:${id}`,
        terms: ["tx", "ty", "tz"].map((dof, index) => ({ nodeId: "b", dof: dof as DofName, coefficient: normal[index]! })),
        rightHandSide: 0,
      });
    }
    const load = 22_000;
  ...
  validationCase({
    ...
    const force = tuple3([0, 1, 2].map((row) =>
    ...
    const global = nodeVector(output, "b", ["tx", "ty", "tz"]);
    return [
      value("local-y displacement", (local[1] * length ** 3) / (3 * E * IZ), dot(global, axes.y), "m"),
      value("local-z displacement", (local[2] * length ** 3) / (3 * E * IY), dot(global, axes.z), "m"),
    ];
  }),
];
...
const supportCases: ValidationCase[] = [
  validationCase({
    ...
    supports: "Left pin restrains tx and ty; right roller restrains ty; unused planar DOFs restrained.",
    ...
    let builder = addStandardProperties(createModelBuilder())
      ...
      .addFrame({ id: "f1", startNodeId: "a", endNodeId: "b", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 1, 0] })
      .addFrame({ id: "f2", startNodeId: "b", endNodeId: "c", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 1, 0] });
    constrain(builder, "a", ["tx", "ty", "tz", "rx", "ry"]);
    constrain(builder, "b", ["tz", "rx", "ry"]);
    constrain(builder, "c", ["ty", "tz", "rx", "ry"]);
    const output = solve(builder.addLoadCase({ id: "LC", loads: [{ kind: "nodal", nodeId: "b", force: [0, -24_000, 0] }] }));
    return [
  ...
  validationCase({
    ...
    return [
      ...
      value("fixed moment magnitude", Math.abs(w) * length ** 2 / 8, Math.abs(reaction(output, "a", "rz")), "N*m"),
    ];
  }),
];
