...
function fix(builder: ModelBuilder, nodeId: string, selected: readonly (typeof dofs)[number][]): void {
  for (const dof of selected) {
    builder.addConstraint({ id: `${nodeId}:${dof}`, terms: [{ nodeId, dof, coefficient: 1 }], rightHandSide: 0 });
  }
}

function cantilever(options: { readonly origin?: readonly [number, number, number]; readonly elasticModulus?: number } = {}) {
  ...
  const builder = createModelBuilder()
    ...
    .addFrameSection({ id: "s", area: 0.02, torsionalConstant: 1e-5, momentOfInertiaY: 3e-5, momentOfInertiaZ: 4e-5 })
    .addFrame({ id: "f", startNodeId: "a", endNodeId: "b", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 1, 0] });
  fix(builder, "a", dofs);
  builder
    .addLoadCase({ id: "P", loads: [{ kind: "nodal", nodeId: "b", force: [1200, -3500, 2700], moment: [300, -500, 800] }] })
    .addLoadCase({ id: "2P", loads: [{ kind: "nodal", nodeId: "b", force: [2400, -7000, 5400], moment: [600, -1000, 1600] }] })
    .addLoadCase({ id: "Fy", loads: [{ kind: "nodal", nodeId: "b", force: [0, 1, 0] }] })
    .addLoadCase({ id: "Mz", loads: [{ kind: "nodal", nodeId: "b", moment: [0, 0, 1] }] })
    .addLoadCase({ id: "FyMz", loads: [{ kind: "nodal", nodeId: "b", force: [0, 1, 0], moment: [0, 0, 1] }] });
  return builder.finalize();
}

function expectScaled(actual: readonly number[], base: readonly number[], scale: number, digits = 10): void {
  expect(actual).toHaveLength(base.length);
  for (let index = 0; index < base.length; index += 1) expect(actual[index]).toBeCloseTo(scale * base[index]!, digits);
}
...
function axialTruss(axis: "x" | "y", reversed = false) {
  ...
  const builder = createModelBuilder()
    ...
    .addTruss({ id: "t", startNodeId: reversed ? "b" : "a", endNodeId: reversed ? "a" : "b", materialId: "m", sectionId: "s" });
  fix(builder, "a", ["tx", "ty", "tz"]);
  fix(builder, "b", axis === "x" ? ["ty", "tz"] : ["tx", "tz"]);
  builder.addLoadCase({ id: "P", loads: [{ kind: "nodal", nodeId: "b", force: axis === "x" ? [12000, 0, 0] : [0, 12000, 0] }] });
  return builder.finalize();
}
...
describe("metamorphic structural properties", () => {
  ...
  it("NFR-COR-001: axial subdivision is equivalent to one unsplit member", () => {
    ...
    const builder = createModelBuilder()
      ...
      .addTruss({ id: "t2", startNodeId: "m", endNodeId: "b", materialId: "mat", sectionId: "sec" });
    fix(builder, "a", ["tx", "ty", "tz"]);
    fix(builder, "m", ["ty", "tz"]);
    fix(builder, "b", ["ty", "tz"]);
    const split = prepareAnalysis(builder.addLoadCase({ id: "P", loads: [{ kind: "nodal", nodeId: "b", force: [12000, 0, 0] }] }).finalize()).solveCase("P");
    expect(split.nodes.find(({ id }) => id === "b")!.displacements[0]!.value).toBeCloseTo(single.nodes.find(({ id }) => id === "b")!.displacements[0]!.value, 14);
  ...
  it("NFR-COR-001: Maxwell-Betti reciprocity holds for force and moment load cases", () => {
    ...
    const displacementFromMoment = moment.nodes[1]!.displacements.find(({ dof }) => dof === "ty")!.value;
  ...
  it("NFR-COR-001: direct superposition equals both a combined load case and result combination", () => {
    ...
    const combined = combineResults("superposed", [{ result: force, factor: 1 }, { result: moment, factor: 1 }]);
