...
function readReference(kind: "euler" | "timoshenko"): Frame3ddReference {
  return JSON.parse(
    readFileSync(join(process.cwd(), `verification/reference-data/frame3dd/cantilever-${kind}/reference.json`), "utf8"),
...
function fixedBuilder(theory: FrameTheoryInput): ModelBuilder {
  ...
  for (const dof of ["tx", "ty", "tz", "rx", "ry", "rz"] as const) {
    builder.addConstraint({ id: `fixed:${dof}`, terms: [{ nodeId: "n1", dof, coefficient: 1 }], rightHandSide: 0 });
  }
  const cases: readonly LoadCaseInput[] = [
    { id: "1", loads: [{ kind: "nodal", nodeId: "n2", force: [1000, -2000, 1500], moment: [100000, 200000, -300000] }] },
    { id: "2", loads: [{ kind: "member-distributed", frameId: "frame", coordinateSystem: "local", startIntensity: [0.2, -0.5, 0.3], endIntensity: [0.2, -0.5, 0.3] }] },
    {
      id: "3",
      loads: [
        { kind: "member-distributed", frameId: "frame", coordinateSystem: "local", startDistanceFromElasticStart: 300, endDistanceFromElasticStart: 1700, startIntensity: [0.05, 0, 0], endIntensity: [0.15, 0, 0] },
        { kind: "member-distributed", frameId: "frame", coordinateSystem: "local", startDistanceFromElasticStart: 400, endDistanceFromElasticStart: 2200, startIntensity: [0, -0.1, 0], endIntensity: [0, -0.6, 0] },
        { kind: "member-distributed", frameId: "frame", coordinateSystem: "local", startDistanceFromElasticStart: 800, endDistanceFromElasticStart: 2600, startIntensity: [0, 0, 0.2], endIntensity: [0, 0, -0.2] },
      ],
    },
    { id: "4", loads: [{ kind: "member-point-force", frameId: "frame", coordinateSystem: "local", distanceFromElasticStart: 1200, force: [100, -200, 300] }] },
...
function prescribedBuilder(theory: FrameTheoryInput): ModelBuilder {
  ...
  for (const [index, dof] of (["tx", "ty", "tz", "rx", "ry", "rz"] as const).entries()) {
    builder.addConstraint({ id: `prescribed:${dof}`, terms: [{ nodeId: "n1", dof, coefficient: 1 }], rightHandSide: values[index]! });
  }
...
function expectArrayClose(actual: ArrayLike<number>, expected: ArrayLike<number>, relative = 2e-5, absolute = 2e-7): void {
  ...
  for (let index = 0; index < expected.length; index += 1) {
    ...
    if (difference > tolerance) throw new Error(`comparison ${index}: actual=${actual[index]} expected=${expected[index]} difference=${difference} tolerance=${tolerance}`);
  }
}
...
function runOracle(kind: "euler" | "timoshenko", theory: FrameTheoryInput): void {
  ...
  it(`${kind}: directly matches Frame3DD element and global stiffness matrices`, () => {
    ...
    expect(reference.oracle.binarySha256).toBe("53b1dc6628424b156e491e3205f13d58a0e325e33e4746d225b456ab85ac5275");
  ...
  for (const expectedCase of reference.cases.slice(0, 5)) {
    it(`${kind}: matches Frame3DD load case ${expectedCase.id}`, () => {
      ...
      if (kind === "euler") {
        expectArrayClose(tip.slice(1), expectedCase.nodeDisplacements[1]!.slice(1));
      } else {
        const yExpected = (-200 * 1200 ** 2 * (3 * 3000 - 1200)) / (6 * 210000 * 6000000) + (-200 * 1200) / (80769.23076923077 * 800);
        const zExpected = (300 * 1200 ** 2 * (3 * 3000 - 1200)) / (6 * 210000 * 8000000) + (300 * 1200) / (80769.23076923077 * 700);
        expect(tip[1]).toBeCloseTo(yExpected, 13);
        expect(tip[2]).toBeCloseTo(zExpected, 13);
        expect(tip[4]).toBeCloseTo((-300 * 1200 ** 2) / (2 * 210000 * 8000000), 13);
        expect(tip[5]).toBeCloseTo((-200 * 1200 ** 2) / (2 * 210000 * 6000000), 13);
      }
