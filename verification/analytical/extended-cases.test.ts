...
function constrain(
  ...
  nodeId: string,
  dofs: readonly (typeof allDofs)[number][],
...
function frameBuilder(
  ...
  return createModelBuilder()
    ...
    .addFrameSection({
      ...
      momentOfInertiaY: inertiaY,
      momentOfInertiaZ: inertiaZ,
    })
    .addFrame({
      id: "f",
      startNodeId: "a",
...
describe("extended analytical verification", () => {
  ...
  it("interior point moment matches exact cantilever tip displacement and rotation", () => {
    ...
    expect(tip.displacements.find(({ dof }) => dof === "rz")!.value).toBeCloseTo(
      (moment * position) / (elasticModulus * inertia),
      12,
    );
