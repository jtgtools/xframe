...
- Frame internal-force stations contain axial force, two shears, torsion, and two bending moments.
- Point forces and point moments create explicit `left` and `right` stations at interior discontinuities.
- Distributed-load span boundaries are included in the station layout.
- Truss results contain extension, strain, axial force, and global end forces.
...
- Result identifiers, model fingerprint, exact unit metadata, conventions, entity IDs/order, active DOF order, spring kind, and frame station layout are checked before arithmetic.
