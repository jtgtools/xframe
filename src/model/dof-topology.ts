...
function request(
  requests: Map<DofKey, { readonly nodeId: EntityId; readonly dof: DofName; readonly sources: Set<string> }>,
  nodeId: EntityId,
  dof: DofName,
  source: string,
): void {
...
function validateConstraintTargets(constraints: readonly ConstraintRecord[], table: PhysicalDofTable): void {
  for (const constraint of constraints) {
    for (let index = 0; index < constraint.terms.length; index += 1) {
      ...
      if (!table.has(createDofKey(term.nodeId, term.dof))) {
        throw new XFrameError("INPUT_INVALID", "Constraint targets a physically unavailable degree of freedom.", {
          kind: "input",
...
export function derivePhysicalDofTopology(model: ModelSnapshot): PhysicalDofTable {
  const requests = new Map<DofKey, { readonly nodeId: EntityId; readonly dof: DofName; readonly sources: Set<string> }>();
  ...
  for (const spring of model.springs) {
    for (let component = 0; component < DOF_NAMES.length; component += 1) {
      ...
      if (spring.endNodeId !== undefined) request(requests, spring.endNodeId, dof, `spring:${spring.id}`);
    }
  }
  ...
  const entries = ordered.map((entry, physicalIndex): PhysicalDofMetadata =>
    Object.freeze({
      ...
      dof: entry.dof,
      physicalIndex,
      requestedBy: Object.freeze([...entry.sources].sort()),
    }),
  );
