...
export function createLoadCombination(input: CombinationInput): CombinationRecord {
  if (!Array.isArray(input.factors) || input.factors.length === 0) {
    throw new XFrameError("INPUT_INVALID", "A load combination requires at least one factor.", {
      kind: "input",
      path: "combination.factors",
      expected: "nonempty array",
      actual: Array.isArray(input.factors) ? String(input.factors.length) : typeof input.factors,
    });
  }
  ...
  const factors = input.factors.map((factor, index) => {
    ...
    if (seen.has(resultId)) {
      throw new XFrameError(
        "INPUT_INVALID",
        "Combination factors cannot repeat a result identifier.",
        {
          kind: "input",
          path: `combination.factors[${index}].resultId`,
          expected: "unique result identifier",
          actual: resultId,
        },
      );
    }
    ...
    if (value === 0) {
      throw new XFrameError("INPUT_INVALID", "Combination factors cannot be zero.", {
        kind: "input",
        path: `combination.factors[${index}].factor`,
        expected: "finite nonzero factor",
        actual: "0",
      });
    }
  ...
  return Object.freeze({
    id: parseIdentifier(input.id, "combination.id"),
