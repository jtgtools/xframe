...
function parseLoad(value: unknown, index: number): LoadRecord {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    loadError(`loadCase.loads[${index}]`, "load object", value === null ? "null" : typeof value);
  const kind = (value as Readonly<Record<string, unknown>>)["kind"];
  switch (kind) {
    case "nodal":
      return createNodalLoad(value as NodalLoadInput);
    case "member-point-force":
    case "member-point-moment":
      return createMemberPointLoad(value as LoadInput & { readonly kind: typeof kind });
    case "member-distributed":
      return createDistributedLoad(value as LoadInput & { readonly kind: typeof kind });
    case "self-weight":
      return createSelfWeightLoad(value as LoadInput & { readonly kind: typeof kind });
    default:
      loadError(`loadCase.loads[${index}].kind`, "recognized load kind", kind);
  }
}

export function createLoadCase(input: LoadCaseInput): LoadCaseRecord {
  for (const key of Object.keys(input))
    if (!ALLOWED.has(key)) loadError(`loadCase.${key}`, "recognized load-case property", key);
  if (input.loads !== undefined && !Array.isArray(input.loads))
    loadError("loadCase.loads", "array", typeof input.loads);
...
function coordinateError(
  ...
  throw new XFrameError(
    "LOAD_INVALID",
    "Member-load coordinate is outside the deformable member.",
    {
      kind: "input",
...
function targetIds(load: ResolvedLoadRecord): readonly EntityId[] {
  switch (load.kind) {
    case "nodal":
      return Object.freeze([load.nodeId]);
    case "member-point-force":
    case "member-point-moment":
    case "member-distributed":
      return Object.freeze([load.frameId]);
    case "self-weight":
      return Object.freeze([...load.frameIds, ...load.trussIds]);
  }
}
      ...
      if (load.kind === "nodal") {
        if (!nodeIds.has(load.nodeId)) {
          throw new XFrameError("REFERENCE_NOT_FOUND", "Nodal load references a missing node.", {
            ...
            issues: [
              {
                entityType: "load case",
                id: loadCase.id,
                path: `loads[${index}].nodeId`,
                referencedId: load.nodeId,
                expectedType: "node",
              },
            ],
          });
        ...
        if (frame === undefined) {
          throw new XFrameError(
            "REFERENCE_NOT_FOUND",
            "Member point load references a missing frame.",
            {
              kind: "reference",
              issues: [
                {
                  entityType: "load case",
                  id: loadCase.id,
                  path: `loads[${index}].frameId`,
                  referencedId: load.frameId,
                  expectedType: "frame",
                },
              ],
            },
        ...
        const distance =
          load.location.kind === "distance"
            ? load.location.distanceFromElasticStart
            : load.location.positionRatio * length;
        if (distance < 0 || distance > length)
          coordinateError(loadCase.id, index, distance, length);
        value =
          load.kind === "member-point-force"
            ? Object.freeze({
                kind: load.kind,
                frameId: load.frameId,
                coordinateSystem: load.coordinateSystem,
                distanceFromElasticStart: distance,
                force: load.force,
                sourceLocation: load.location,
              })
            : Object.freeze({
                kind: load.kind,
                frameId: load.frameId,
        ...
        if (frame === undefined) {
          throw new XFrameError(
            "REFERENCE_NOT_FOUND",
            "Distributed load references a missing frame.",
            {
              kind: "reference",
              issues: [
                {
        ...
        const start =
          load.span.kind === "full"
            ? 0
            : load.span.kind === "distance"
              ? load.span.startDistanceFromElasticStart
              : load.span.startPositionRatio * length;
        const end =
          load.span.kind === "full"
            ? length
            : load.span.kind === "distance"
              ? load.span.endDistanceFromElasticStart
              : load.span.endPositionRatio * length;
        if (start < 0 || end > length || end <= start)
          coordinateError(loadCase.id, index, end > length ? end : start, length);
        value = Object.freeze({
          kind: load.kind,
          frameId: load.frameId,
          coordinateSystem: load.coordinateSystem,
        ...
        const selectedFrames =
          load.frameIds === undefined
            ? hasExplicitSelection
              ? []
              : frames
            : load.frameIds.map((id) => {
                const frame = frameById.get(id);
                if (frame === undefined) {
                  throw new XFrameError(
                    "REFERENCE_NOT_FOUND",
                    "Self-weight references a missing frame.",
                    {
                      kind: "reference",
                      issues: [
                        {
        ...
        const selectedTrusses =
          load.trussIds === undefined
            ? hasExplicitSelection
              ? []
              : trusses
            : load.trussIds.map((id) => {
                const truss = trussById.get(id);
                if (truss === undefined) {
                  throw new XFrameError(
                    "REFERENCE_NOT_FOUND",
                    "Self-weight references a missing truss.",
                    {
                      kind: "reference",
                      issues: [
                        {
        ...
        if (selectedFrames.length + selectedTrusses.length === 0)
          loadError(
            `loadCases[${loadCase.id}].loads[${index}]`,
            "at least one affected frame or truss",
            "none",
          );
        const missing = [
          ...selectedFrames.map((entry) => entry.material),
          ...selectedTrusses.map((entry) => entry.material),
        ]
        ...
        if (missing.length > 0) {
          throw new XFrameError(
            "MATERIAL_INVALID",
            "Self-weight requires density for every affected material.",
            {
              kind: "material-dependency",
              loadCaseId: loadCase.id,
              missingMaterialIds: Object.freeze([...new Set(missing)].toSorted()),
              affectedElementIds: Object.freeze(
                [
                  ...selectedFrames.map(({ record }) => record.id),
                  ...selectedTrusses.map(({ record }) => record.id),
                ].toSorted(),
              ),
        ...
        value = Object.freeze({
          ...
          frameIds: Object.freeze(selectedFrames.map(({ record }) => record.id).toSorted()),
          trussIds: Object.freeze(selectedTrusses.map(({ record }) => record.id).toSorted()),
        });
      }
      resolved.push(value);
      provenance.push(
