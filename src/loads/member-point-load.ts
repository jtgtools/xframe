...
const FORCE_KEYS = new Set(["kind", "frameId", "coordinateSystem", "distanceFromElasticStart", "positionRatio", "force"]);
const MOMENT_KEYS = new Set(["kind", "frameId", "coordinateSystem", "distanceFromElasticStart", "positionRatio", "moment"]);
...
function location(input: MemberPointLoadInput): MemberLocation {
  ...
  if (hasDistance === hasRatio) loadError("load", "exactly one of distanceFromElasticStart or positionRatio", `${hasDistance},${hasRatio}`);
  if (hasDistance) {
    const distance = finite(input.distanceFromElasticStart, "load.distanceFromElasticStart");
    if (distance < 0) loadError("load.distanceFromElasticStart", "nonnegative physical distance", distance);
...
export function createMemberPointLoad(input: MemberPointLoadInput): MemberPointLoadRecord {
  ...
  if (input.kind === "member-point-moment") {
    ...
    if (!nonzero(moment)) loadError("load.moment", "at least one nonzero component", moment.join(","));
    return Object.freeze({
      kind: input.kind,
      frameId: parseIdentifier(input.frameId, "load.frameId"),
      coordinateSystem: coordinateSystem(input.coordinateSystem, "load.coordinateSystem"),
  ...
  loadError("load.kind", "member-point-force or member-point-moment", (input as { readonly kind?: unknown }).kind);
}
