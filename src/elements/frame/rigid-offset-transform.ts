...
function endpointBlock(globalToLocal: ArrayLike<number>, offsetInput: ArrayLike<number>): Float64Array {
  ...
  for (let block = 0; block < 2; block += 1) {
    for (let row = 0; row < 3; row += 1) {
      for (let column = 0; column < 6; column += 1) {
        let value = 0;
        for (let inner = 0; inner < 3; inner += 1) value += globalToLocal[row * 3 + inner]! * h[(inner + block * 3) * 6 + column]!;
        result[(row + block * 3) * 6 + column] = finiteNumber(value, `frameTransform[${row + block * 3},${column}]`);
      }
    }
  }
  return result;
  ...
  return Object.freeze({
    ...
    toLocalDisplacements(globalNodalDisplacements: ArrayLike<number>): Float64Array {
      ...
      for (let row = 0; row < 12; row += 1) {
        let value = 0;
        for (let column = 0; column < 12; column += 1) value += matrix[row * 12 + column]! * input[column]!;
    ...
    stiffnessToGlobal(localStiffnessInput: ArrayLike<number>): Float64Array {
      ...
      for (let row = 0; row < 12; row += 1) {
        for (let column = 0; column < 12; column += 1) {
          let value = 0;
          for (let inner = 0; inner < 12; inner += 1) value += local[row * 12 + inner]! * matrix[inner * 12 + column]!;
      ...
      for (let row = 0; row < 12; row += 1) {
        for (let column = 0; column < 12; column += 1) {
          let value = 0;
          for (let inner = 0; inner < 12; inner += 1) value += matrix[inner * 12 + row]! * intermediate[inner * 12 + column]!;
          result[row * 12 + column] = finiteNumber(value, `frame.globalStiffness[${row},${column}]`);
        }
      }
      return result;
    },
