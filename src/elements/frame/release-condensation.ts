...
function checkedMatrix(values: ArrayLike<number>, length: number, path: string): Float64Array {
  ...
  for (let index = 0; index < length; index += 1) result[index] = finiteNumber(values[index], `${path}[${index}]`);
  return result;
}

function factorPositiveDefinite(matrix: number[][]): number[][] {
  const size = matrix.length;
  const factor = Array.from({ length: size }, () => new Array<number>(size).fill(0));
  const scale = Math.max(1, ...matrix.map((row, index) => Math.abs(row[index]!)));
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column <= row; column += 1) {
      let value = matrix[row]![column]!;
      for (let inner = 0; inner < column; inner += 1) value -= factor[row]![inner]! * factor[column]![inner]!;
      if (row === column) {
        if (!Number.isFinite(value) || value <= 256 * Number.EPSILON * scale) {
          throw new XFrameError("ELEMENT_LOCAL_MECHANISM", "Released frame DOFs form a singular local block.", {
            kind: "analysis",
...
function solveFactor(factor: number[][], rhs: readonly number[]): number[] {
  ...
  const value = [...rhs];
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column < row; column += 1) value[row] = value[row]! - factor[row]![column]! * value[column]!;
    value[row] = value[row]! / factor[row]![row]!;
  }
  for (let row = size - 1; row >= 0; row -= 1) {
    value[row] = value[row]! / factor[row]![row]!;
    for (let column = 0; column < row; column += 1) value[column] = value[column]! - factor[row]![column]! * value[row]!;
  }
  return value;
}
...
export function frameReleaseMask(releases: FrameReleaseInput | undefined): number {
  ...
  let mask = 0;
  for (const [offset, values] of [[0, releases.start], [6, releases.end]] as const) {
...
export function condenseFrameEndReleases(
  ...
  const releasedDofs = Array.from({ length: 12 }, (_, index) => index).filter((index) => (releaseMask & (1 << index)) !== 0);
  const retainedDofs = Array.from({ length: 12 }, (_, index) => index).filter((index) => (releaseMask & (1 << index)) === 0);
  ...
  const kqq = releasedDofs.map((row) => releasedDofs.map((column) => originalStiffness[row * 12 + column]!));
  const factor = factorPositiveDefinite(kqq);
  const inverseKqr = retainedDofs.map((retained) =>
    solveFactor(factor, releasedDofs.map((released) => originalStiffness[released * 12 + retained]!)),
  );
  const loadOffset = solveFactor(factor, releasedDofs.map((released) => originalLoad[released]!));
  ...
  for (let rowIndex = 0; rowIndex < retainedDofs.length; rowIndex += 1) {
    ...
    for (let inner = 0; inner < releasedDofs.length; inner += 1) loadValue -= originalStiffness[row * 12 + releasedDofs[inner]!]! * loadOffset[inner]!;
    ...
    for (let columnIndex = 0; columnIndex < retainedDofs.length; columnIndex += 1) {
      ...
      for (let inner = 0; inner < releasedDofs.length; inner += 1) value -= originalStiffness[row * 12 + releasedDofs[inner]!]! * solvedColumn[inner]!;
  ...
  return Object.freeze({
    ...
    recoverLocalDisplacements(values: ArrayLike<number>): Float64Array {
      ...
      for (let releasedIndex = 0; releasedIndex < releasedDofs.length; releasedIndex += 1) {
        ...
        for (let retainedIndex = 0; retainedIndex < retainedDofs.length; retainedIndex += 1) value -= inverseKqr[retainedIndex]![releasedIndex]! * result[retainedDofs[retainedIndex]!]!;
        result[releasedDofs[releasedIndex]!] = finiteNumber(value, `releasedDisplacement[${releasedIndex}]`);
      }
      return result;
    },
  });
