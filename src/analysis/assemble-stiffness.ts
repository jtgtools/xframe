import { compileModelConstraints, type CompiledConstraints } from "../constraints/compile-constraints.js";
...
import { SymmetricCoordinateBuilder, type SymmetricCoordinateMatrix } from "../linalg/symmetric-coordinate-matrix.js";
import type { FinalizedModel } from "../model/finalized-model.js";
import { activeSpringComponents, frameEquationMap, springEquationMap, trussEquationMap } from "./element-equation-map.js";
...
function reducedMatrix(full: SymmetricCoordinateMatrix, constraints: CompiledConstraints): SymmetricCoordinateMatrix {
...
export function assembleStiffness(model: FinalizedModel): AssembledStiffness {
  ...
  for (const frame of model.resolvedFrames) {
    ...
    addDenseSymmetric(fullBuilder, frameEquationMap(model, frame), transform.stiffnessToGlobal(condensed.stiffness));
  }
  ...
  for (const truss of model.resolvedTrusses) {
    ...
    addDenseSymmetric(fullBuilder, trussEquationMap(model, truss), stiffness);
  }

  for (const spring of model.resolvedSprings) {
    const components = activeSpringComponents(spring);
    const full = spring.record.endNodeId === undefined
    ...
    const indexes = spring.record.endNodeId === undefined
      ? components
