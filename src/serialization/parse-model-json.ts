...
function nodes(value: unknown): readonly NodeInput[] {
  return arrayAt(value, "$.nodes").map((entry, index) => {
    ...
    return { id: id(record["id"], `${path}.id`), coordinates: finiteVector(record["coordinates"], `${path}.coordinates`, 3) };
  });
}
...
function materials(value: unknown): readonly MaterialInput[] {
  return arrayAt(value, "$.materials").map((entry, index) => {
    ...
    const record = objectAt(entry, path, ["id", "elasticModulus", "shearModulus", "poissonRatio", "density"], ["id"]);
    return {
      id: id(record["id"], `${path}.id`),
      ...(record["elasticModulus"] === undefined ? {} : { elasticModulus: finiteAt(record["elasticModulus"], `${path}.elasticModulus`) }),
      ...(record["shearModulus"] === undefined ? {} : { shearModulus: finiteAt(record["shearModulus"], `${path}.shearModulus`) }),
      ...(record["poissonRatio"] === undefined ? {} : { poissonRatio: finiteAt(record["poissonRatio"], `${path}.poissonRatio`) }),
      ...(record["density"] === undefined ? {} : { density: finiteAt(record["density"], `${path}.density`) }),
    };
  });
}
...
function frameSections(value: unknown): readonly FrameSectionInput[] {
  ...
  return arrayAt(value, "$.frameSections").map((entry, index) => {
    ...
    return {
      ...
      ...(record["shearAreaY"] === undefined ? {} : { shearAreaY: finiteAt(record["shearAreaY"], `${path}.shearAreaY`) }),
      ...(record["shearAreaZ"] === undefined ? {} : { shearAreaZ: finiteAt(record["shearAreaZ"], `${path}.shearAreaZ`) }),
    };
  });
}
...
function rigidOffsets(value: unknown, path: string): NonNullable<FrameInput["rigidOffsets"]> {
  ...
  return {
    ...(record["start"] === undefined ? {} : { start: finiteVector(record["start"], `${path}.start`, 3) }),
...
function frames(value: unknown): readonly FrameInput[] {
  ...
  return arrayAt(value, "$.frames").map((entry, index) => {
    ...
    const record = objectAt(entry, path, [...required, "orientation", "releases", "rigidOffsets"], required);
    ...
    if (record["releases"] !== undefined) {
      ...
      const parseEnd = (key: "start" | "end") => releaseRecord[key] === undefined
        ? undefined
        : arrayAt(releaseRecord[key], `${path}.releases.${key}`).map((dof, dofIndex) => literalAt(dof, `${path}.releases.${key}[${dofIndex}]`, DOFS));
      const start = parseEnd("start");
      const end = parseEnd("end");
      releases = { ...(start === undefined ? {} : { start }), ...(end === undefined ? {} : { end }) };
    }
    return {
      id: id(record["id"], `${path}.id`),
      ...
      theory: { kind: literalAt(theory["kind"], `${path}.theory.kind`, ["euler-bernoulli", "timoshenko"]) },
      ...(record["orientation"] === undefined ? {} : { orientation: finiteVector(record["orientation"], `${path}.orientation`, 3) }),
      ...(releases === undefined ? {} : { releases }),
      ...(record["rigidOffsets"] === undefined ? {} : { rigidOffsets: rigidOffsets(record["rigidOffsets"], `${path}.rigidOffsets`) }),
    };
  });
}
...
function springs(value: unknown): readonly SpringInput[] {
  return arrayAt(value, "$.springs").map((entry, index) => {
    ...
    const record = objectAt(entry, path, ["id", "startNodeId", "endNodeId", "stiffness"], ["id", "startNodeId", "stiffness"]);
    return {
      id: id(record["id"], `${path}.id`),
      startNodeId: id(record["startNodeId"], `${path}.startNodeId`),
      ...(record["endNodeId"] === undefined ? {} : { endNodeId: id(record["endNodeId"], `${path}.endNodeId`) }),
...
function constraints(value: unknown): readonly ConstraintInput[] {
  return arrayAt(value, "$.constraints").map((entry, index) => {
    ...
    return { id: id(record["id"], `${path}.id`), terms, rightHandSide: finiteAt(record["rightHandSide"], `${path}.rightHandSide`) };
  });
}

function vectorProperty(record: Readonly<Record<string, unknown>>, key: string, path: string): readonly number[] | undefined {
...
function load(value: unknown, path: string): LoadInput {
  ...
  const kind = literalAt(kindRecord["kind"], `${path}.kind`, ["nodal", "member-point-force", "member-point-moment", "member-distributed", "self-weight"]);
  if (kind === "nodal") {
    const record = objectAt(value, path, ["kind", "nodeId", "force", "moment"], ["kind", "nodeId"]);
    return { kind, nodeId: id(record["nodeId"], `${path}.nodeId`), ...(vectorProperty(record, "force", path) === undefined ? {} : { force: vectorProperty(record, "force", path)! }), ...(vectorProperty(record, "moment", path) === undefined ? {} : { moment: vectorProperty(record, "moment", path)! }) };
  }
  if (kind === "member-point-force" || kind === "member-point-moment") {
    const vectorKey = kind === "member-point-force" ? "force" : "moment";
    const record = objectAt(value, path, ["kind", "frameId", "coordinateSystem", "distanceFromElasticStart", "positionRatio", vectorKey], ["kind", "frameId", "coordinateSystem", vectorKey]);
    const common = {
      kind,
      frameId: id(record["frameId"], `${path}.frameId`),
      coordinateSystem: literalAt(record["coordinateSystem"], `${path}.coordinateSystem`, ["local", "global"]),
      ...(record["distanceFromElasticStart"] === undefined ? {} : { distanceFromElasticStart: finiteAt(record["distanceFromElasticStart"], `${path}.distanceFromElasticStart`) }),
      ...(record["positionRatio"] === undefined ? {} : { positionRatio: finiteAt(record["positionRatio"], `${path}.positionRatio`) }),
    };
  ...
  if (kind === "member-distributed") {
    const record = objectAt(value, path, ["kind", "frameId", "coordinateSystem", "startDistanceFromElasticStart", "endDistanceFromElasticStart", "startPositionRatio", "endPositionRatio", "startIntensity", "endIntensity"], ["kind", "frameId", "coordinateSystem", "startIntensity", "endIntensity"]);
    return {
      kind,
      frameId: id(record["frameId"], `${path}.frameId`),
      coordinateSystem: literalAt(record["coordinateSystem"], `${path}.coordinateSystem`, ["local", "global"]),
      ...(record["startDistanceFromElasticStart"] === undefined ? {} : { startDistanceFromElasticStart: finiteAt(record["startDistanceFromElasticStart"], `${path}.startDistanceFromElasticStart`) }),
      ...(record["endDistanceFromElasticStart"] === undefined ? {} : { endDistanceFromElasticStart: finiteAt(record["endDistanceFromElasticStart"], `${path}.endDistanceFromElasticStart`) }),
      ...(record["startPositionRatio"] === undefined ? {} : { startPositionRatio: finiteAt(record["startPositionRatio"], `${path}.startPositionRatio`) }),
      ...(record["endPositionRatio"] === undefined ? {} : { endPositionRatio: finiteAt(record["endPositionRatio"], `${path}.endPositionRatio`) }),
  ...
  const record = objectAt(value, path, ["kind", "gravity", "frameIds", "trussIds"], ["kind", "gravity"]);
  const ids = (key: "frameIds" | "trussIds") => record[key] === undefined
    ? undefined
    : arrayAt(record[key], `${path}.${key}`).map((entry, index) => id(entry, `${path}.${key}[${index}]`));
  const frameIds = ids("frameIds");
  const trussIds = ids("trussIds");
  return { kind, gravity: finiteVector(record["gravity"], `${path}.gravity`, 3), ...(frameIds === undefined ? {} : { frameIds }), ...(trussIds === undefined ? {} : { trussIds }) };
}
...
function loadCases(value: unknown): readonly LoadCaseInput[] {
  return arrayAt(value, "$.loadCases").map((entry, index) => {
    ...
    return { id: id(record["id"], `${path}.id`), loads: arrayAt(record["loads"], `${path}.loads`).map((entry, loadIndex) => load(entry, `${path}.loads[${loadIndex}]`)) };
  });
}
  ...
  return arrayAt(value, "$.combinations").map((entry, index) => {
    ...
    return {
      ...
      factors: arrayAt(record["factors"], `${path}.factors`).map((factor, factorIndex) => {
        ...
        return { resultId: id(factorRecord["resultId"], `${factorPath}.resultId`), factor: finiteAt(factorRecord["factor"], `${factorPath}.factor`) };
      }),
    };
  });
}
...
export function parseModelJson(input: unknown): FinalizedModel {
  ...
  if (root["schemaVersion"] !== MODEL_SCHEMA_VERSION) {
    schemaError("$.schemaVersion", `schema version ${MODEL_SCHEMA_VERSION}`, root["schemaVersion"], "SCHEMA_UNSUPPORTED", String(root["schemaVersion"]));
  }
