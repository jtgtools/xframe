...
function physicalBuilder() {
  return createModelBuilder()
    ...
    .addFrameSection({ id: "fs", area: 0.01, torsionalConstant: 1e-5, momentOfInertiaY: 2e-5, momentOfInertiaZ: 3e-5 })
    .addTrussSection({ id: "ts", area: 0.002 })
    .addFrame({ id: "f", startNodeId: "n2", endNodeId: "n3", materialId: "m", sectionId: "fs", theory: { kind: "euler-bernoulli" }, orientation: [0, 0, 1] })
...
describe("physical DOF topology", () => {
  it("FR-DOF-001: derives deterministic physical DOFs from element kinematics", () => {
    ...
    const actual = Array.from(model.physicalDofs.values(), ({ nodeId, dof, physicalIndex }) => `${physicalIndex}:${nodeId}.${dof}`);
    expect(actual).toEqual([
      "0:n1.tx", "1:n1.ty", "2:n1.tz", "3:n1.rz",
      "4:n2.tx", "5:n2.ty", "6:n2.tz", "7:n2.rx", "8:n2.ry", "9:n2.rz",
      "10:n3.tx", "11:n3.ty", "12:n3.tz", "13:n3.rx", "14:n3.ry", "15:n3.rz",
    ]);
  ...
  it("FR-DOF-002: truss-only nodes have translations and no rotations", () => {
    ...
    expect(Array.from(model.physicalDofs.values(), ({ dof }) => dof)).toEqual(["tx", "ty", "tz", "tx", "ty", "tz"]);
  ...
  it("FR-DOF-001: rejects constraints that target physically unavailable DOFs", () => {
    const builder = createModelBuilder()
      ...
      .addTrussSection({ id: "ts", area: 0.002 })
      .addTruss({ id: "t", startNodeId: "a", endNodeId: "b", materialId: "m", sectionId: "ts" })
      .addConstraint({ id: "c", terms: [{ nodeId: "a", dof: "rx", coefficient: 1 }], rightHandSide: 0 });
