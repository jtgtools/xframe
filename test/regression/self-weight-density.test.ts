...
it("FR-MAT-002/FR-LOD-006: self-weight aggregates every affected material missing density", () => {
  const builder = createModelBuilder()
    ...
    .addNode({ id: "a", coordinates: [0, 0, 0] }).addNode({ id: "b", coordinates: [1, 0, 0] }).addNode({ id: "c", coordinates: [2, 0, 0] })
    .addMaterial({ id: "m-frame", elasticModulus: 1, shearModulus: 1 })
    .addMaterial({ id: "m-truss", elasticModulus: 1, shearModulus: 1 })
    .addFrameSection({ id: "fs", area: 1, torsionalConstant: 1, momentOfInertiaY: 1, momentOfInertiaZ: 1 })
    .addTrussSection({ id: "ts", area: 1 })
    .addFrame({ id: "f", startNodeId: "a", endNodeId: "b", materialId: "m-frame", sectionId: "fs", theory: { kind: "euler-bernoulli" } })
  ...
  try { builder.finalize(); throw new Error("expected failure"); } catch (error) {
    ...
    expect((error as XFrameError).code).toBe("MATERIAL_INVALID");
    expect((error as XFrameError).context).toEqual({
      kind: "material-dependency", loadCaseId: "SW", missingMaterialIds: ["m-frame", "m-truss"], affectedElementIds: ["f", "t"], reason: "self-weight requires density",
    });
  }
});
