...
describe("integrated case solves", () => {
  it("FR-SOL-004/FR-RES-001: solves an axial truss and recovers its support reaction", () => {
    ...
    const model = builder.addLoadCase({ id: "P", loads: [{ kind: "nodal", nodeId: "b", force: [15, 0, 0] }] }).finalize();
  ...
  it("FR-ELE-001/FR-SOL-004: solves a fixed-base Euler cantilever tip load", () => {
    ...
    const builder = createModelBuilder()
      ...
      .addFrameSection({ id: "s", area: 0.01, torsionalConstant: 1e-5, momentOfInertiaY: inertia, momentOfInertiaZ: inertia })
      .addFrame({ id: "f", startNodeId: "base", endNodeId: "tip", materialId: "m", sectionId: "s", theory: { kind: "euler-bernoulli" }, orientation: [0, 1, 0] });
    for (const dof of ["tx", "ty", "tz", "rx", "ry", "rz"] as const) builder.addConstraint({ id: `base:${dof}`, terms: [{ nodeId: "base", dof, coefficient: 1 }], rightHandSide: 0 });
    const model = builder.addLoadCase({ id: "P", loads: [{ kind: "nodal", nodeId: "tip", force: [0, -12_000, 0] }] }).finalize();
    const result = prepareAnalysis(model).solveCase("P");
    const tipY = model.physicalDofs.get(createDofKey("tip", "ty"))!.physicalIndex;
    expect(result.fullDisplacements[tipY]).toBeCloseTo((-12_000 * length ** 3) / (3 * elasticModulus * inertia), 11);
  });
